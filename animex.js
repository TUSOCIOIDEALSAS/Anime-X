...
const VIP_CODES=['ANIMEX-VIP-2026','VIPGOLD-ANIMEX','HECTOR-VIP'];
...
function activateVip(){
  ...
  if(!VIP_CODES.includes(code)){
    err.textContent='Código inválido. Verifica tu compra.';
    err.style.display='block';
    return;
  }
  const expiry=Date.now()+(30*24*60*60*1000);
  const updated={...currentUser,vip:true,vipExpiry:expiry};
  UserDB.updateUser(updated);
  currentUser=updated;
  updateNavUser();
  closeVipModal();
  toast('¡VIP activado por 30 días! ♛');
}
...
function showPage(id){
  if((id==='community'||id==='profile'||id==='vip')&&!currentUser){
    openAuthModal('login');
    toast('Inicia sesión para acceder',true);
    return;
  }
...
function renderCommunity(){
  ...
  // Selector de anime para filtrar
...
function submitPost(){
  ...
  commAnimeFilter=animeId;
  renderCommunity();
}

function likePost(id){
  if(!currentUser){openAuthModal('login');return}
  CommDB.toggleLike(id,currentUser.id);
  renderCommunity();
}

function deletePost(id){
  if(!currentUser)return;
  CommDB.delete(id,currentUser.id);
  renderCommunity();
}
...
function renderProfile(){
  ...
  document.getElementById('profile-content').innerHTML=`
    <div class="profile-card">
      <div class="profile-avatar">${currentUser.avatar}
        ${vipActive?'<div class="profile-vip-crown">♛</div>':''}
      </div>
...
function showLoginAdmin(){
  ...
  document.getElementById('admin-login-err').classList.remove('on');
...
function switchATab(t){
  ...
  const idx={alist:0,aadd:1,astats:2}[t];
...
function addEpRow(){
  ...
  tempEps.push({num:tempEps.length+1,season:lastSeason,title:'',url:''});
  renderEpsAdmin();
}

function removeEp(i){
  tempEps.splice(i,1);tempEps.forEach((ep,j)=>ep.num=j+1);renderEpsAdmin();
}
